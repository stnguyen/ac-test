...
describe("isCityValid", () => {
  it("should return true if the population exceed the threshold", () => {
    expect(isCityValid({ population: "4900" } as any, 5000)).toBeFalsy();
    expect(isCityValid({ population: "5100" } as any, 5000)).toBeTruthy();
    expect(isCityValid({ population: "54x9" } as any, 5000)).toBeFalsy();
  });
});
...
describe("indexData", () => {
  it("should transform a tsv file to an array of objects", async () => {
    const data = await indexCities("/path/to");
    expect(data).toEqual({
      cities: [
        {
...
describe("build a text index", () => {
  it("should index correctly the text index", () => {
    ...
    buildTextIndex(index as any, { canonicalName: "query", id: "1" } as any);
    buildTextIndex(index as any, { canonicalName: "quera", id: "2" } as any);
    buildTextIndex(index as any, { canonicalName: "query0", id: "3" } as any);
